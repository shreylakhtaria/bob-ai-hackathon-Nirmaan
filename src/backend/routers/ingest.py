...
# ---------------------------------------------------------------------------
@router.get("/template")
def download_template(current=Depends(require_any_role)):
...
async def ingest_assets_csv(file: UploadFile = File(...), current=Depends(require_operator)):
    ...
    if valid:
        ...
        actor = current.get("email", "operator") if isinstance(current, dict) else "operator"
        db.audit(actor, "asset_upsert_csv",
...
async def ingest_assets_json(records: List[AssetRecord], current=Depends(require_operator)):
    ...
    actor = current.get("email", "operator") if isinstance(current, dict) else "operator"
    db.audit(actor, "asset_upsert_json",
...
async def ingest_telemetry(packet: TelemetryPacket, current=Depends(require_any_role)):
...
def re_score(req: RescoreRequest = None, current=Depends(require_operator)):
    """
    ...
    actor = current.get("email", "operator") if isinstance(current, dict) else "operator"
    db.audit(actor, "re_score", {"scored": scored, "areas_updated": areas_updated, "as_of": as_of})
...
async def ingest_asset_single(record: AssetRecord, current=Depends(require_operator)):
    """Add or update a single asset. Identical to /assets/json with one record."""
    inserted, updated = _upsert_assets([record])
    actor = current.get("email", "operator") if isinstance(current, dict) else "operator"
    db.audit(actor, "asset_upsert_single",
...
def seed_telemetry(req: SeedTelemetryRequest = None, current=Depends(require_operator)):
    """
